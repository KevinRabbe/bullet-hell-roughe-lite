...
func _spawn_elite(spawn_position: Vector2) -> Node:
	var elite_variant := _pick_elite_variant()
	...
	if enemy_node != null and log_portal_events:
		print("Spawned elite variant: %s" % elite_variant)
	return enemy_node
...
func _pick_elite_variant() -> String:
	return PortalEventManagerRuntime.pick_elite_variant(rng)
